...
where ``<SIGNATORY_VERSION>`` is the version of Signatory you would like to download (the most recent version is 1.3.0) and ``<TORCH_VERSION>`` is the version of PyTorch you are using.

Available for Python 3.12 on Linux and Windows. Requires `PyTorch <http://pytorch.org/>`__ 2.1.0+.
...
For example, if you are using PyTorch 2.2.0 and want Signatory 1.3.0, then you should run:

.. code-block:: bash

    pip install signatory==1.3.0.2.2.0 --no-cache-dir --force-reinstall
