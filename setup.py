...
try:
    ...
    raise ImportError(
        "PyTorch is not installed, and must be installed prior to installing Signatory."
    )
...
# Custom BuildExtension to use new C++ ABI for compatibility with newer GCC
class BuildExtension(cpp.BuildExtension):
    def build_extensions(self):
        # Replace -D_GLIBCXX_USE_CXX11_ABI=0 with -D_GLIBCXX_USE_CXX11_ABI=1
        # The old ABI causes segfaults with GCC 11+ and newer PyTorch versions
        for ext in self.extensions:
            if hasattr(ext, "extra_compile_args"):
                ext.extra_compile_args = [
                    "-D_GLIBCXX_USE_CXX11_ABI=1"
                    if arg == "-D_GLIBCXX_USE_CXX11_ABI=0"
                    else arg
