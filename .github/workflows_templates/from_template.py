...
def _substitute(filename, **subs):
    ...
    with io.open(
        os.path.join(here, filename + ".template"), encoding="utf-8", mode="r"
    ) as f:
    ...
    for line in template_lines:
        ...
        if found_argument_header:
            ...
            if argument_name in subs:
                raise RuntimeError(
                    "Argument {} already in subs for filename {}".format(
    ...
    while True:
        ...
        for sub_key_bracket, (sub_re, sub_val_split) in subs_re.items():
            ...
            first_characters = template[
                searched.start() : searched.start() + white_space_amount
            ]
            white_sub_vals = [first_characters + sub_val_split[0]]
            white_sub_vals.extend(
                [white_space + sub_val for sub_val in sub_val_split[1:]]
            )
            template = (
                template[: searched.start()]
                + "\n".join(white_sub_vals)
                + template[searched.end() :]
            )
        if not found:
            break
    ...
    if search:
        raise RuntimeError(
            "Found unsubbed string {} in {}".format(search.group(), filename)
        )
    ...
    with io.open(
        os.path.join(here, "..", "workflows", filename), encoding="utf-8", mode="w"
    ) as f:
        f.write(template)
...
global_subs = dict(
  ...
  <<event_name>>:""",
    ...
    pytorch211="2.1.1",
    pytorch220="2.2.0",
    pytorch_all="[<<pytorch210>>, <<pytorch211>>, <<pytorch220>>]",
...
strategy:
  ...
  fail-fast: false""",
    ...
    strategy_single="""runs-on: ${{ matrix.os }}
strategy:
  matrix:
    os: [<<linux>>]
    python-version: [<<py312>>]
    pytorch-version: [<<pytorch220>>]
""",
    # A single Linux strategy except with all PyTorch versions
    strategy_single_all_pytorch="""runs-on: ${{ matrix.os }}
strategy:
  matrix:
    os: [<<linux>>]
    python-version: [<<py312>>]
...
""",
    ...
    _action_pv_312="(contains(github.event.action, '-pv <<py312>>') && matrix.python-version == '<<py312>>')",
    _action_pv_star="contains(github.event.action, '-pv *')",
    action_pv="(<<_action_pv_312>> || <<_action_pv_star>>)",
    ...
    checkout_code="""name: Checkout code
<<if_>>
uses: actions/checkout@v1""",
    # A step to install Python 3.12. NOTE THAT IT IS DELIBERATELY ONLY 3.12.
    ...
    install_python="""name: Install Python
<<if_>>
uses: actions/setup-python@v1
with:
  python-version: '3.12'""",
    # Performs the necessary set-up for Windows.
    # Needs the cpuonly package because Windows is a bit weird.
    # Signatory on Linux with cpuonly installed will still allow for Signatory to run on the GPU.
    setup_windows=r"""name: Windows
...
run: >
  ...
  python command.py should_not_import &&""",
    ...
    build_windows="""  python setup.py egg_info --tag-build=".${{ matrix.pytorch-version }}" bdist_wheel &&
  python command.py should_not_import &&""",
    ...
    install_remote_windows="""  python -c "import subprocess;
  import sys;
  import time;
  import metadata;
  ...
  " &&""",
    ...
    test_windows=r"""  python -m pip install numpy &&
...
run: |
  ...
  python command.py should_not_import""",
    # 'Builds' on Linux
    build_linux="""  python setup.py egg_info --tag-build=".${{ matrix.pytorch-version }}" sdist
  python command.py should_not_import""",
    # Install from sdist or bdist_wheel on Linux
    install_local_linux="""  SIGNATORY_INSTALLED=$(python -c \"import os
  import sys
  x = os.listdir('dist')
  print(x[0])
  sys.exit(len(x) != 1)\")
  python -m pip install ./dist/$SIGNATORY_INSTALLED""",
    ...
    install_remote_linux="""  retry () { $* || (sleep 20 && $*) || (sleep 40 && $*) || (sleep 120 && $*) || (sleep 240 && $*); }
  ...
  retry python -m pip install <<install_extras>>signatory==$SIGNATORY_VERSION.${{ matrix.pytorch-version }} --no-binary signatory""",
    # Runs tests on Linux
    test_linux=r"""  python -m pip install numpy
  python -m pip install iisignature pytest
  python -c "import os
  import subprocess
  import sys
